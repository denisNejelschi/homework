...
public class Seller extends Person {
  ...
  public Seller(String name) {
    ...
    products[0] = new Product("Eggs", 24, 0.5);
    ...
    products[5] = new Product("Beer", 24, 0.9);
    ...
    for (int i = 0; i < products.length; i++) {
      if (products[i].getName().equals(name)) {
        if (products[i].getQuantity() >= amount) {
          System.out.println("Seller: the price is " + amount * products[i].getPrice() );
          return amount * products[i].getPrice();
        }
        break;
      }
    }
    System.out.println(" Seller: Sorry we don't have it ");
    return -1;
